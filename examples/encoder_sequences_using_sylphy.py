...
import pandas as pd 
from sylphy.sequence_encoder import OneHotEncoder
from sylphy.sequence_encoder import OrdinalEncoder
from sylphy.sequence_encoder import FrequencyEncoder
...
kmer_train = KMersEncoders(
    dataset=df_training,
    sequence_column="seq")

kmer_test = KMersEncoders(
    dataset=df_testing,
    sequence_column="seq")
...
phy_train = PhysicochemicalEncoder(
    dataset=df_training,
    sequence_column="seq",
    max_length=548)

phy_test = PhysicochemicalEncoder(
    dataset=df_testing,
    sequence_column="seq",
    max_length=548)
